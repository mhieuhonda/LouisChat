  ...
  void _onNewMessage(Map<String, dynamic> payload) {
    ...
    if (msgJson is Map<String, dynamic>) {
      ...
      if (msg.conversationId == widget.conversationId) {
        ...
        _scrollToBottom();
        // If the message is from the peer, mark read immediately
        if (msg.senderId != widget.peerId && mounted) {
          // my own echo - nothing
        } else {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          ...
          IconButton(
            icon: const Icon(Icons.more_horiz, color: MessengerTheme.textPrimary),
            onPressed: () {},
          ),
        ],
      ),
        ...
        child: Row(
          children: [
            IconButton(
              ...
              onPressed: () {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Đính kèm ảnh/video sẽ có ở v0.2')),
                );
              },
            ),
            IconButton(
              ...
              onPressed: () {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Camera sẽ có ở v0.2')),
                );
              },
            ),
            Expanded(
